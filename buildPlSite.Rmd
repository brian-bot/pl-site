```{r setup, include=FALSE, results='hide'}
knitr::opts_chunk$set(warning = FALSE, message = FALSE)
source("code/populateStats.R")
source("code/calculatePeriodStats.R")
```

## Leaderboard: period `r currentPeriod`

```{r leagueStats, echo=FALSE, results='asis'}
require(knitr)
teamNames <- names(allRosters[[names(currentPeriod)]])

tmpDF <- leagueStats[[ currentPeriod ]]
tmpDF$team <- paste0("[", tmpDF$team, "](./", paste0(gsub(" ", "", tolower(tmpDF$team))), ")")
kable(tmpDF, row.names = FALSE)
```

* * *
* * *

```{r teamStats, include=FALSE, results='hide'}
for(i in teamNames){
  cat(paste0("### ", i, "\n\n"),
      kable(allRosters[[names(currentPeriod)]][[i]]$battingStats, caption="batters", row.names = FALSE),
      "\n\n* * *\n\n",
      kable(allRosters[[names(currentPeriod)]][[i]]$pitchingStats, caption="pitchers", row.names = FALSE),
      "\n\n* * *\n\n",
      fill=TRUE,
      file=paste0(gsub(" ", "", tolower(i)), ".md"))
}
```
